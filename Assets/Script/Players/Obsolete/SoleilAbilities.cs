...
public class SoleilAbilities : MonoBehaviour
{
    [Header("F. Attributes")]
    ...
    void Update()
    {
        ...
        if ((Input.GetButtonDown("SFire") && Input.GetButton("SHeal") || (Input.GetButtonDown("SHeal") && Input.GetButton("SFire")) && BlastTimer >= TimeBetweenBlast))
        {
        ...
        if (Input.GetButton("SFire") && fireCountdown <= 0f)
        {
            if (blasting == false)
            {
                ShootingAnimation();
        ...
        else if (Input.GetButton("SHeal") && timer >= TimeBetweenHealth)
        {
            if (blasting == false)
            {
                HealingAnimation();
